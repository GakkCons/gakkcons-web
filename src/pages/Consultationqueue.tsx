...
function ConsultationQueue() {
  ...
  const toggleDoneModal = (appointment_id: number | null = null) => {
    setSelectedAppointmentId(appointment_id);
    setShowDoneModal((prev) => !prev);
  };
  ...
  const handleDone = async () => {
    ...
    try {
      const response = await axios.put(`/appointments/completed/${selectedAppointmentId}`, JSON.stringify(statusData), {
        headers: headers,
      });
  ...
  return (
    <>
      ...
      <div className="mx-2 sm:mx-4 md:mx-10 details mt-4 mb-2">
        <div className="grid grid-cols-1 md:grid-cols-12 lg:grid-cols-12 gap-4">
          ...
          >
            ...
            <div className="p-2 sm:p-6 md:p-3"
              style={{
                width: '100%',
                height: '70vh',
                maxHeight: '100%',
                ...
                {Object.keys(sortedGroupedAppointments).length === 0 ? (
                  ...
                  Object.keys(sortedGroupedAppointments).map((date, index) => (
                    <div key={index}>
                      <h2 className="text-lg font-bold mb-2">{date}</h2>
                      <div className="overflow-x-auto">
                        {sortedGroupedAppointments[date].map((request: any) => (
                          <div key={request.appointment_id} className="h-auto p-1 bg-[#d1c8c3] rounded-lg mb-2">
                            <div className="grid grid-cols-6 gap-4 textc items-center m-2">
                              <div className="col-span-2 flex items-center gap-4">
                                <div className="p-3 border-2 border-black rounded-full">
                              ...
                              <div className="flex justify-center">
                                <button
                                  className="px-3 py-1 rounded-md bg-green-700 text-white"
                                  aria-label="Mark as done"
                                  onClick={() => toggleDoneModal(request.appointment_id)}
                                >
                                  Done
                                </button>
                              </div>
      ...
      {/* Done Modal */}
      {showDoneModal && (
        <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50" style={{ backdropFilter: 'blur(1px)' }}>
          <div className="bg-white p-6 rounded-lg max-w-sm w-full">
            <h2 className="text-center text-lg font-semibold mb-4">Are you sure you want to complete this request?</h2>
            <div className="flex justify-center space-x-4">
              <button onClick={handleDone} className="bg-red-500 text-white py-2 px-4 rounded-md hover:bg-red-700">Confirm</button>
              <button onClick={toggleDoneModal} className="bg-gray-300 text-black py-2 px-4 rounded-md hover:bg-gray-500">Cancel</button>
            </div>
          </div>
        </div>
      )}
