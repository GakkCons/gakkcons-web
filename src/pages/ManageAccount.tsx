...
function ManageAccount() {
  ...
  const { data: subjects, error: subjectError, isLoading: isSubjetsLoading, refetch: subjectRefetch } = useQuery({
    queryKey: ['subjects'],
  ...
  const { data: users, error: usersError, isLoading: isUsersLoading, refetch: isUsersRefetch } = useQuery({
    queryKey: ['users'],
  ...
  useEffect(() => {
    if (users?.data) {
      ...
      if (selectedRole) {
        setFilteredUsers(users.data.filter((user) => user.role_name === selectedRole));
      } else {
  ...
  const handleRegisterTeacher = () => {
    axios.post('users/signupadmin', data, {
      headers: {
        'Authorization': `Bearer ${token}`,
      },
    })
                  ...
                  {step === 'select' ? (
                    ...
                    <>
                      <h2 className="text-xl font-bold mb-4">
                        Register {data.userType.charAt(0).toUpperCase() + data.userType.slice(1)} Account
                      </h2>
...
{/* 
  ...
  <input
    ...
    onChange={(e) =>
      setSelectedUser({ ...selectedUser, password: e.target.value }) // Update selectedUser password
    }
  />
</div>
                        ...
                        {selectedUser.role_name === 'faculty' && (
                          <div className="mb-4">
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setSelectedUser({ ...selectedUser, subject_id: e.target.value })
                              }
                            >
                              <option value="">Select a subject</option>
